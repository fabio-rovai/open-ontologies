...
| TCB-15, TCB-16, TCB-17 | property, and **proved unbounded of a MODEL** | `src/reason.rs` unit property tests, `dafny/Interner.dfy` | the interner is private. Kani cannot reach these at all: a harness over `Interner` returns 5,108 of 5,109 checks undetermined on `CCRandomGenerateBytes`, which is `std`'s `HashMap` seeding `RandomState` from the OS, and fixing the hasher does not help because the obstacle is `HashMap` rather than the seed. Dafny proves all three for every string and every number of insertions, OF A REIMPLEMENTATION AND NOT OF THIS RUST, which is decision 0014 and the row means nothing without it |
