...
- Known reality-check points are concentrated in T13 (e2e): `onto_load` argument names and `onto_query` output shape. Both have explicit correction instructions. The `onto_diff` (`old_path`/`new_path`, verified in engine `src/server.rs`) names are confirmed; `onto_pack`/`onto_unpack` argument names should be confirmed against `OntoPackInput`/`OntoUnpackInput` in engine `src/server.rs` during Task 9 and corrected in place if they differ from `{path}`.
