...
You have access to the Open Ontologies MCP server, which provides 119 tools for AI-native ontology engineering backed by an in-memory Oxigraph triple store. All 116 are available in a default build, but 8 require an optional Cargo feature and return an error without it: four need `embeddings`, two need `plugins`, and two need `postgres` or `duckdb`.
