...
| `onto_temporal_conflicts` | To separate genuine contradictions from corrections. A disjointness violation only counts when the two assertions claim OVERLAPPING validity; where one period ends as the other begins, the pair is superseded history, not a conflict |
