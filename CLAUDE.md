...
| `onto_temporal_snapshot` | To see which named graphs are in scope at a point in time. Two independent clocks: `valid_at` asks what was TRUE then, `as_of` asks what was KNOWN then. Graphs without validity metadata are timeless and always in scope, so the vocabulary is additive to an existing store |
...
| `onto_temporal_conflicts` | To separate genuine contradictions from pairs that never met. A disjointness violation only counts when the two assertions claim OVERLAPPING validity; everything else lands in `non_overlapping`, which proves only that the two share no instant under a LEXICAL comparison of their bounds (offsets are not normalised) — not that one replaced the other, and it also holds pairs separated by a gap, which is missing coverage rather than history. `superseded` is the same set under its old name, deprecated and dropped at 2.0 |
