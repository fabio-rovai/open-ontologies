...
| `onto_temporal_conflicts` | To separate genuine contradictions from pairs that never met. A disjointness violation only counts when the two assertions claim OVERLAPPING validity; everything else lands in `non_overlapping`, which proves only that the two are never asserted at the same instant — not that one replaced the other, and it also holds pairs separated by a gap, which is missing coverage rather than history. `superseded` is the same set under its old name, deprecated and dropped at 2.0 |
