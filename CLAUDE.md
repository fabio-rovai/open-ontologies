...
| `onto_justify` | AFTER a conclusion surprises you, and BEFORE deleting an axiom to fix a contradiction. Axiom pinpointing: the MINIMAL sets of asserted triples responsible for a triple (`triple`) or for a clash (`inconsistency: true`). A support set that is not minimal blames axioms that had nothing to do with the conclusion, so minimality is the product. Three claims, three words: SUFFICIENCY is re-run and, with `certificate_dir`, machine-checkable by `oo-cert` under `OOCert.certificate_sound`; MINIMALITY is re-run without each element and is covered by NO theorem; COMPLETENESS of the list is Reiter's hitting-set tree, bounded by `max_justifications` (16) and `max_oracle_calls` (400), with `truncated` naming the bound that fired, and complete only for a monotone oracle. Pass `candidate` to CHECK a set instead of searching for one: it comes back `minimal_justification`, `not_a_justification_not_minimal` with the useless triples named, or `not_a_justification_target_not_reached`. For a clash the word is `clash_found_by_this_engine` and never the checker's. `owl-dl` is refused: no rule trace, no derivation to walk. See docs/explanation.md |
| `onto_provenance` | To see WHY a triple holds as algebra rather than as a list. Provenance semirings over the whole derivation DAG, not over `derivations.tsv`, which records only the first derivation of each triple: `boolean`, `why` (whose minimal monomials are the justifications), `lineage` (the union of contributors, which is NOT a justification), `counting`, `tropical` (min-plus, the cheapest proof tree) and `trust` (max-min, the confidence of the weakest premise). RECURSION IS NAMED RATHER THAN HIDDEN: `counting` diverges on a cycle, so round k counts proof trees of height at most k, `depth_bound` (32) rides beside the number, `value_is_exact` is false unless the iteration stabilised, and `cycle_in_support` names a triple on the cycle. `why` is capped by `max_monomials` (64), the cap keeps the SMALLEST monomials so the survivors stay an antichain, and a truncated run withdraws the minimality claim. Negative weights are refused for min-plus and weights above 1.0 for max-min, because each breaks the semiring. Nothing here is machine-checked; `onto_justify` is the one that re-runs the engine. See docs/explanation.md |
