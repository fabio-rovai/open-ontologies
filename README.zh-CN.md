...
证书**不**记录它来自哪个图。证书证明的是：结论可以从证书中列出的断言推出。它无法告诉你，
那些断言就是你数据库里的断言。这个缺口是
[issue #158](https://github.com/fabio-rovai/open-ontologies/issues/158)，而且这个缺口是
开放的。绿色结果什么也证明不了，因为任何程序都能打印 `ok`。重要的行为是检查器会变红。
